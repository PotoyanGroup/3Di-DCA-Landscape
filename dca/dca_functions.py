# ref: https://github.com/utdal/py-mfdca.git
import numpy as np 
...
def load_couplings(N, q, eij_file):
    ...
    if couplings.shape[1] == q*q:
        ...
        exit('The couplings do not have a dim of q*q (q*q ij block, flattened). Please reformat your data!')
    ...
    for i in prange(N-1):
        for j in range(i+1,N):
            for l in range(M):
                ...
                Pij[j,i,sequences[l,j],sequences[l,i]] = Pij[i,j,sequences[l,i],sequences[l,j]]
...
@jit(nopython=True)
def add_pseudocount(Pi,Pij,pseudocount_weight,N,q): #add pseudocount to counting matrices 
    Pij_pc = (1 - pseudocount_weight) * Pij +  pseudocount_weight / q / q * np.ones((N,N,q,q),dtype=np.float64)
    Pi_pc = (1 - pseudocount_weight) * Pi + pseudocount_weight / q * np.ones((N,q),dtype=np.float64)
    ...
    for i in range(N):
        for alpha in range(q):
            for beta in range(q):
                    Pij_pc[i,i,alpha,beta] = (1-pseudocount_weight)*Pij[i,i,alpha,beta] + pseudocount_weight / q * qq_id[alpha,beta]
    return Pi_pc, Pij_pc
        ...
        for j in range(N):
            for alpha in range(q-1):
                for beta in range(q-1):
                     C[(q-1)*(i)+alpha,(q-1)*(j)+beta] = Pij[i,j,alpha,beta] - Pi[i,alpha]*Pi[j,beta]
    return C
...
def Compute_Results(Pi, invC, N, q): # calculate DI score for each pair of positions
    ...
    for i in range(N):
        for j in range(i+1,N):
            ...
            diff = 1.0
            mu1 = np.ones(q,dtype=np.float64) / q; mu2 = np.ones(q,dtype=np.float64) / q
            pi = Pi[i,:] ; pj = Pi[j,:]
            while diff > epsilon:
...
def Compute_AverageLocalField(Pairwisehfield,N,q): # caluclate local fields (h)
    ...
    for i in prange(N):
        pairwise_chunk = ( Pairwisehfield[ i*q: (i*q)+q , range(0,N*2,2) ].sum(1) - Pairwisehfield[ i*q: (i*q)+q , i ] ) / ( N - 1 )
        hi[:,i] = pairwise_chunk
    return hi 

@njit(parallel=True)
def return_Hamiltonian(numerical_sequences, couplings, localfields, N1=""): # calculates Hamiltonian for input fasta
    ...
    for l in prange(M):
        for i in range(L):
            hamiltonians[l] = hamiltonians[l] + localfields[numerical_sequences[l, i], i ]
    for l in prange(M):
        for i in prange(L-1):
            for j in range(i+1,L):
                hamiltonians[l] = hamiltonians[l] + couplings[ i , j , numerical_sequences[l,i] , numerical_sequences[l,j] ] 
